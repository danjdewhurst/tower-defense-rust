...
pub fn bullet_movement(
    ...
    for (entity, mut transform, bullet) in bullet_query.iter_mut() {
        transform.translation += Vec3::new(bullet.direction.x, bullet.direction.y, 0.0) * bullet.speed * time.delta_secs();
        ...
        for (enemy_entity, mut enemy, enemy_transform) in enemy_query.iter_mut() {
            ...
            if distance < 16.0 { // Collision threshold
                ...
                commands.spawn((
                    ...
                    Transform::from_translation(enemy_transform.translation + Vec3::new(0.0, 0.0, 2.0)),
                    HitEffect { timer: Timer::from_seconds(0.2, TimerMode::Once) },
                ));
